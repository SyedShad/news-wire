...
export async function GET(request: Request) {
  ...
  return noStore(NextResponse.json({
    ok: true,
    settings: stored?.snapshot.settings || {},
    diagnostics: stored?.snapshot.diagnostics || [],
    bridge: {
      connected: Boolean(stored?.bridgeConnected),
      last_seen_at: stored?.bridgeLastSeenAt || null,
      version: stored?.bridgeVersion || null,
      error: stored?.bridgeLastError || null,
    },
  }));
}
