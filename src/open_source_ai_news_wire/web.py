...
def create_app(
    *,
    ...
    def shared_context() -> dict[str, Any]:
        ...
        assistance = assistance_status(database)
        assistance_notice: dict[str, str] | None = None
        if not (assistance["enabled"] and assistance["ready"]):
            reason = str(assistance.get("failure_reason") or "")
            if reason == "codex_authentication_unavailable":
                title = "ChatGPT login required"
                remediation = "Sign in with the local Codex command, then run the assistance security check."
            elif reason.startswith("codex_identity") or reason.startswith("codex_"):
                title = "Bundled ChatGPT drafting command unavailable"
                remediation = "Use the signed Codex command bundled with ChatGPT, then rerun the assistance security check."
            elif reason:
                title = "Assistance security check required"
                remediation = "Run the assistance isolation check, then enable assisted drafting."
            else:
                title = "Assisted drafting is ready but off"
                remediation = "Enable assisted drafting when you want approved drafts generated by ChatGPT."
            assistance_notice = {
                "title": title,
                "message": remediation,
                "reason": reason,
        ...
        return {
            ...
            "assistance_notice": assistance_notice,
        }
