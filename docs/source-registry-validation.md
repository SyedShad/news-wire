...
`validation_status` records whether a definition passed the live check, is `validated-unavailable`, or is `excluded-by-operator`. `evidence_use` records primary, reporting, or discovery use. `trust_class` explicitly records `trusted` or `research_required`; it controls immediate scoring and automatic research but never bypasses safe networking.
...
On 2026-07-23, HuggingNews was revalidated against its documented anonymous `GET /api/stories` interface and exact-slug detail interface. The JSON adapter stores `eventTimeApprox` or `publishedAt` as aggregator discovery time, never as the linked article's original publication time. The linked page is inspected separately for publication metadata; until that succeeds the story is `Newly surfaced` with zero freshness points. Selected public posts and safely resolved links are deduplicated Discovery leads. HuggingNews is `research_required` in schema 8, so these signals trigger automatic research rather than being treated as trusted publication evidence. Authenticated pagination, private Convex fields, and API keys remain out of scope.
